...
export function parseChangeModeOutput(qwenResponse: string): ChangeModeEdit[] {
  const edits: ChangeModeEdit[] = [];
  const markdownPattern = /\*\*FILE:\s*(.+?):(\d+)\*\*\s*\n```\s*\nOLD:\s*\n([\s\S]*?)\nNEW:\s*\n([\s\S]*?)\n```/g;

  let match;
  while ((match = markdownPattern.exec(qwenResponse)) !== null) {
  ...
  if (edits.length === 0) {
    ...
    while ((match = editPattern.exec(qwenResponse)) !== null) {
      const [
        _fullMatch,
        oldFilename,
        oldStartLine,
