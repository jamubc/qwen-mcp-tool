...
`;
  ...
  try {
    ...
    if (errorMessage.includes(ERROR_MESSAGES.QUOTA_EXCEEDED) && model !== MODELS.QWEN_WEAK_MODEL) {
      Logger.warn(`${ERROR_MESSAGES.QUOTA_EXCEEDED}. Falling back to ${MODELS.QWEN_WEAK_MODEL}.`);
      await sendStatusMessage(STATUS_MESSAGES.QWEN_WEAK_MODEL_RETRY);
      const fallbackArgs = [];
      fallbackArgs.push(CLI.FLAGS.MODEL, MODELS.QWEN_WEAK_MODEL);
      if (sandbox) {
      ...
      try {
        ...
        Logger.warn(`Successfully executed with ${MODELS.QWEN_WEAK_MODEL} fallback.`);
        await sendStatusMessage(STATUS_MESSAGES.QWEN_WEAK_MODEL_SUCCESS);
        return result;
      } catch (fallbackError) {
        const fallbackErrorMessage = fallbackError instanceof Error ? fallbackError.message : String(fallbackError);
        throw new Error(`${MODELS.QWEN_STRONG_MODEL} quota exceeded, ${MODELS.QWEN_WEAK_MODEL} fallback also failed: ${fallbackErrorMessage}`);
      }
    } else {
      throw error;
    }
